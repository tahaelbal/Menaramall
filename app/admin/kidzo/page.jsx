...
export default function KidzoAdmin() {
  ...
  // Charger tous les kidzos
  const fetchKidzos = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/kidzo`);
      const data = await res.json();
      const kidzoArray = Array.isArray(data) ? data : [];
      setKidzos(kidzoArray);
    } catch (error) {
      console.error("Erreur fetch kidzos :", error);
      setKidzos([]);
    }
    setLoading(false);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let res;
      if (editingId) {
        res = await fetch(`/api/kidzo/${editingId}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(body),
        });
      } else {
        res = await fetch("/api/kidzo", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(body),
        });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`/api/kidzo/${id}`, { method: "DELETE" });
      if (!res.ok) {
        const errorData = await res.json();
